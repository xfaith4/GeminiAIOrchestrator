...
const PlanDisplay: React.FC<PlanDisplayProps> = ({
  ...
  const awaitingApproval = planState === 'awaitingApproval';
  ...
  return (
    <div className="bg-base-200/70 border border-base-300 rounded-lg flex flex-col h-full">
      ...
      <div className="p-4 space-y-3 overflow-y-auto flex-grow">
        {isGenerating && <Loader message="Supervisor is generating plan..." />}

        {!isGenerating && plan.length === 0 && (
          <p className="text-content-200 text-sm">
            No steps yet. Provide a goal and generate a plan.
          </p>
        )}
        ...
        {plan.map((step, index) => {
          ...
          return (
            // Use a UNIQUE key to avoid React warnings when step.step repeats
            <div
              key={`${step.step}-${index}`}
              className={`p-3 rounded-md transition-all ${
                isCurrent
                  ? 'bg-brand-primary/30 ring-1 ring-brand-secondary'
                  : isCompleted
                  ? 'bg-green-500/10'
                  : 'bg-base-200'
              }`}
            >
              <div className="flex items-start gap-3">
                {isCompleted ? (
                  ...
                  <div
                    className={`w-6 h-6 flex-shrink-0 rounded-full flex items-center justify-center font-bold text-sm ${
                      isCurrent
                ...
                <div className="flex-1">
                  <p
                    className={`font-semibold ${
                      isCurrent ? 'text-white' : 'text-content-100'
                    }`}
                  >
                    {step.task}
                  </p>
